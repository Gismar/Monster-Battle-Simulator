...
namespace Battle
{
    public abstract class Monster : IMonster
    {
        ...
        {
            Health = MathF.Max(MathF.Min(health, Max.health), Min.health);
            Power = MathF.Max(MathF.Min(power, Max.power), Min.power);
            Defence = MathF.Max(MathF.Min(defence, Max.defence), Min.defence);
            Speed = MathF.Max(MathF.Min(speed, Max.speed), Min.speed);
            _random = new Random();
